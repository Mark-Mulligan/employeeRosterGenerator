...
const generalQuestions = [{
    ...
    {
        type: 'input',
        message: "Enter the Emloyee's id:",
        name: 'id'
    },
    {
        type: 'input',
...
const internQuestion = [{
    type: 'input',
    message: 'What is their school do they attend?',
    name: 'school'
}]
...
function getEmployeeInfo(employeeType) {
    inquirer.prompt(generalQuestions).then((res) => {
        if (employeeType === 'Manager') {
            inquirer.prompt(managerQuestion)
                .then((data) => {
                    employees.push(new Manager(res.name, res.id, res.email, data.officeNum));    
                    addMemberPrompt();             
                });
        } else if (employeeType === 'Engineer') {
            inquirer.prompt(engineerQuestion)
                .then((data) => {
                    employees.push(new Engineer(res.name, res.id, res.email, data.githubUsername)); 
                    addMemberPrompt();   
                });
        } else {
